...
import { StrategySortedLinkedList } from "../strategy-sorted-linked-list/strategy-sorted-linked-list";
...
export class StrategySortedLinkedSet<T> implements Set<T> {
  private readonly values: StrategySortedLinkedList<T>;
  ...
  constructor(
    ...
    if (resolvedComparator === undefined && initialValues.length === 0) {
      throw new ComparatorInferenceError(
        "Cannot create StrategySortedLinkedSet without a comparator or initial elements",
      );
    }
    ...
    this.values = new StrategySortedLinkedList<T>(
      resolvedComparator,
      [],
