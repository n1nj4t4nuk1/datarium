...
import { LambdaSortedLinkedList } from "../../lists/lambda-sorted-linked-list/lambda-sorted-linked-list";
...
export class LambdaSortedLinkedSet<T> implements Set<T> {
  private readonly values: LambdaSortedLinkedList<T>;
  ...
  constructor(
    ...
    if (resolvedComparator === undefined && initialValues.length === 0) {
      throw new ComparatorInferenceError(
        "Cannot create LambdaSortedLinkedSet without a comparator or initial elements",
      );
    }
    ...
    this.values = new LambdaSortedLinkedList<T>(
      resolvedComparator,
      [],
