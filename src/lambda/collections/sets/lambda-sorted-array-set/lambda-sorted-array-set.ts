...
import { LambdaSortedArrayList } from "../../lists/lambda-sorted-array-list/lambda-sorted-array-list";
...
export class LambdaSortedArraySet<T> implements Set<T> {
  private readonly values: LambdaSortedArrayList<T>;
  ...
  constructor(
    ...
    if (resolvedComparator === undefined && initialValues.length === 0) {
      throw new ComparatorInferenceError(
        "Cannot create LambdaSortedArraySet without a comparator or initial elements",
      );
    }
    ...
    this.values = new LambdaSortedArrayList<T>(
      resolvedComparator,
      [],
