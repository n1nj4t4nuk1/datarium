...
describe("StrategySortedArrayList with basic types", () => {
  test("infers BasicNumberComparator from first element", () => {
    const list = new StrategySortedArrayList<number>(undefined, [4, 1, 3, 2]);
  ...
  test("throws ComparatorInferenceError when no comparator and no initial elements", () => {
    expect(() => {
      new StrategySortedArrayList<number>();
    }).toThrow(ComparatorInferenceError);
  });

  test("keeps numbers sorted ascending with explicit comparator", () => {
    const list = new StrategySortedArrayList<number>((left, right) => left - right);
  ...
  test("set keeps sorted order after replacing element", () => {
    const list = new StrategySortedArrayList<number>((left, right) => left - right, [1, 3, 5]);
  ...
  test("throws IndexOutOfBoundsError when set uses invalid index", () => {
    const list = new StrategySortedArrayList<number>((left, right) => left - right, [1]);
  ...
  test("uses inferred equality comparator for number contains/indexOf/remove", () => {
    const list = new StrategySortedArrayList<number>(undefined, [3, 1, 2]);
  ...
  test("uses custom equality comparator when provided", () => {
    const list = new StrategySortedArrayList<{ id: number; label: string }>(
