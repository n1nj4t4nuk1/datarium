...
describe("StrategyBoundedSortedLinkedList with class type User", () => {
  ...
  test("orders users by age in constructor", () => {
    const list = new StrategyBoundedSortedLinkedList<User>(
      3,
      orderByAge,
      [
        new User(1, "Ana", 35),
  ...
  test("returns false when remove does not find a matching element", () => {
    const list = new StrategyBoundedSortedLinkedList<User>(
      3,
      orderByAge,
      [
    ...
    expect(list.remove(new User(999, "Ghost", 99))).toBe(false);
  });
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    ...
    expect(() => list.add(new User(3, "Marta", 29))).toThrow(ListCapacityExceededError);
  });
});
