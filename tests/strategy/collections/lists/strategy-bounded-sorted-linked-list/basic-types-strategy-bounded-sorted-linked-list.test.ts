...
import { StrategyBoundedSortedLinkedList } from "../../../../../src/strategy/collections/lists/strategy-bounded-sorted-linked-list/strategy-bounded-sorted-linked-list";
import { ComparatorInferenceError } from "../../../../../src/strategy/errors/comparator-inference-error";

describe("StrategyBoundedSortedLinkedList with basic types", () => {
  test("infers BasicNumberComparator from first element", () => {
    const list = new StrategyBoundedSortedLinkedList<number>(4, undefined, [4, 1, 3, 2]);
  ...
  test("throws ComparatorInferenceError when no comparator and no initial elements", () => {
    expect(() => {
      new StrategyBoundedSortedLinkedList<number>(3);
    }).toThrow(ComparatorInferenceError);
  });

  test("keeps numbers sorted ascending with explicit comparator", () => {
    const list = new StrategyBoundedSortedLinkedList<number>(3, (left, right) => left - right);
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    const list = new StrategyBoundedSortedLinkedList<number>(2, (left, right) => left - right, [2, 1]);
  ...
  test("throws ListCapacityExceededError when initial elements exceed capacity", () => {
    expect(() => new StrategyBoundedSortedLinkedList<number>(2, undefined, [3, 2, 1])).toThrow(
      ListCapacityExceededError,
    );
  });

  test("set keeps sorted order after replacing element", () => {
    const list = new StrategyBoundedSortedLinkedList<number>(3, (left, right) => left - right, [1, 3, 5]);
  ...
  test("uses inferred equality comparator for contains/indexOf/remove", () => {
    const list = new StrategyBoundedSortedLinkedList<number>(5, undefined, [3, 1, 2]);
  ...
  test("throws IndexOutOfBoundsError for invalid indexes", () => {
    const list = new StrategyBoundedSortedLinkedList<number>(1, (left, right) => left - right, [1]);
  ...
  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new StrategyBoundedSortedLinkedList<number>(-1)).toThrow(InvalidCapacityError);
    expect(() => new StrategyBoundedSortedLinkedList<number>(1.5)).toThrow(InvalidCapacityError);
  });
});
