...
import { StrategyBoundedLinkedList } from "../../../../../src/strategy/collections/lists/strategy-bounded-linked-list/strategy-bounded-linked-list";
...
describe("StrategyBoundedLinkedList (unsorted)", () => {
  test("creates an empty list with fixed capacity", () => {
    const list = new StrategyBoundedLinkedList<number>(3);
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    const list = new StrategyBoundedLinkedList<number>(2, [1, 2]);

    expect(() => list.add(3)).toThrow(ListCapacityExceededError);
  });

  test("addAt inserts at explicit index", () => {
    const list = new StrategyBoundedLinkedList<number>(3, [1, 3]);
  ...
  test("set replaces element at index", () => {
    const list = new StrategyBoundedLinkedList<number>(3, [1, 2, 3]);
  ...
  test("contains/indexOf/remove use default equality behavior", () => {
    const list = new StrategyBoundedLinkedList<string>(3, ["a", "b", "c"]);
  ...
  test("throws IndexOutOfBoundsError for invalid indexes", () => {
    const list = new StrategyBoundedLinkedList<number>(1, [1]);
  ...
  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new StrategyBoundedLinkedList<number>(-1)).toThrow(InvalidCapacityError);
    expect(() => new StrategyBoundedLinkedList<number>(1.5)).toThrow(InvalidCapacityError);
  });
});
