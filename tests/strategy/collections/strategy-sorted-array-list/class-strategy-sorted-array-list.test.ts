...
describe("StrategySortedArrayList with class type User", () => {
  ...
  test("orders users by age in constructor", () => {
    const list = new StrategySortedArrayList<User>(
      orderByAge,
      [
        new User(1, "Ana", 35),
        new User(2, "Luis", 22),
  ...
  test("keeps users ordered by age when adding elements", () => {
    const list = new StrategySortedArrayList<User>(orderByAge, [new User(1, "Ana", 35)], equalById);
