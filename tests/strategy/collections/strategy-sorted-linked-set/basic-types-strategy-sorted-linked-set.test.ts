...
import { StrategySortedLinkedSet } from "../../../../src/strategy/collections/strategy-sorted-linked-set/strategy-sorted-linked-set";
...
describe("StrategySortedLinkedSet with basic types", () => {
  test("infers BasicNumberComparator from first element", () => {
    const set = new StrategySortedLinkedSet<number>(undefined, [4, 1, 3, 2]);
  ...
  test("throws ComparatorInferenceError when no comparator and no initial elements", () => {
    expect(() => {
      new StrategySortedLinkedSet<number>();
    }).toThrow(ComparatorInferenceError);
  });

  test("throws DuplicateElementError when adding duplicate values", () => {
    const set = new StrategySortedLinkedSet<number>((left, right) => left - right);
  ...
  test("rejects duplicate values during initialization", () => {
    expect(() => {
      new StrategySortedLinkedSet<number>(undefined, [3, 1, 2, 1]);
  ...
  test("clear removes all elements", () => {
    const set = new StrategySortedLinkedSet<number>((left, right) => left - right, [1, 2, 3]);
  ...
  test("uses custom equality comparator when provided", () => {
    const set = new StrategySortedLinkedSet<{ id: number; label: string }>(
