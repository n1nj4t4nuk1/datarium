...
describe("StrategyBoundedSortedLinkedSet with basic types", () => {
  test("infers BasicNumberComparator from first element", () => {
    const set = new StrategyBoundedSortedLinkedSet<number>(4, undefined, [4, 1, 3, 2]);
  ...
  test("throws ComparatorInferenceError when no comparator and no initial elements", () => {
    expect(() => {
      new StrategyBoundedSortedLinkedSet<number>(3);
    }).toThrow(ComparatorInferenceError);
  });

  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new StrategyBoundedSortedLinkedSet<number>(-1, (left, right) => left - right)).toThrow(
      InvalidCapacityError,
    );

    expect(() => new StrategyBoundedSortedLinkedSet<number>(1.5, (left, right) => left - right)).toThrow(
      InvalidCapacityError,
    );
  });
  ...
  test("keeps numbers sorted ascending with explicit comparator", () => {
    const set = new StrategyBoundedSortedLinkedSet<number>(3, (left, right) => left - right);
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    const set = new StrategyBoundedSortedLinkedSet<number>(2, (left, right) => left - right, [2, 1]);
  ...
  test("throws ListCapacityExceededError when initial elements exceed capacity", () => {
    expect(() => new StrategyBoundedSortedLinkedSet<number>(2, undefined, [3, 2, 1])).toThrow(
  ...
  test("isEmpty returns correct value", () => {
    const set = new StrategyBoundedSortedLinkedSet<number>(1, (left, right) => left - right);
  ...
  test("clear removes all elements", () => {
    const set = new StrategyBoundedSortedLinkedSet<number>(3, (left, right) => left - right, [1, 2, 3]);
  ...
  test("uses custom equality comparator when provided", () => {
    const set = new StrategyBoundedSortedLinkedSet<{ id: number; label: string }>(
      3,
