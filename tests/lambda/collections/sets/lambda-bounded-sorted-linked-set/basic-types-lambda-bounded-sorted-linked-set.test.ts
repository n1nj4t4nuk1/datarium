...
describe("LambdaBoundedSortedLinkedSet with basic types", () => {
  test("infers BasicNumberComparator from first element", () => {
    const set = new LambdaBoundedSortedLinkedSet<number>(4, undefined, [4, 1, 3, 2]);
  ...
  test("throws ComparatorInferenceError when no comparator and no initial elements", () => {
    expect(() => {
      new LambdaBoundedSortedLinkedSet<number>(3);
    }).toThrow(ComparatorInferenceError);
  });

  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new LambdaBoundedSortedLinkedSet<number>(-1, (left, right) => left - right)).toThrow(
      InvalidCapacityError,
    );

    expect(() => new LambdaBoundedSortedLinkedSet<number>(1.5, (left, right) => left - right)).toThrow(
      InvalidCapacityError,
    );
  });

  test("ignores duplicate values when adding", () => {
    const set = new LambdaBoundedSortedLinkedSet<number>(4, (left, right) => left - right);
  ...
  test("ignores duplicate values during initialization", () => {
    const set = new LambdaBoundedSortedLinkedSet<number>(4, undefined, [3, 1, 2, 1]);
  ...
  test("keeps numbers sorted ascending with explicit comparator", () => {
    const set = new LambdaBoundedSortedLinkedSet<number>(3, (left, right) => left - right);
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    const set = new LambdaBoundedSortedLinkedSet<number>(2, (left, right) => left - right, [2, 1]);
  ...
  test("throws ListCapacityExceededError when initial elements exceed capacity", () => {
    expect(() => new LambdaBoundedSortedLinkedSet<number>(2, undefined, [3, 2, 1])).toThrow(
      ListCapacityExceededError,
    );
  });

  test("contains returns true for existing elements and false for missing", () => {
    const set = new LambdaBoundedSortedLinkedSet<number>(5, (left, right) => left - right, [1, 3, 5]);
  ...
  test("isEmpty returns correct value", () => {
    const set = new LambdaBoundedSortedLinkedSet<number>(1, (left, right) => left - right);
  ...
  test("clear removes all elements", () => {
    const set = new LambdaBoundedSortedLinkedSet<number>(3, (left, right) => left - right, [1, 2, 3]);
  ...
  test("uses custom equality comparator when provided", () => {
    const set = new LambdaBoundedSortedLinkedSet<{ id: number; label: string }>(
      3,
