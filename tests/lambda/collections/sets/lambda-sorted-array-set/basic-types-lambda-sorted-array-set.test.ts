...
import { LambdaSortedArraySet } from "../../../../../src/lambda/collections/sets/lambda-sorted-array-set/lambda-sorted-array-set";
import { ComparatorInferenceError } from "../../../../../src/lambda/errors/comparator-inference-error";

describe("LambdaSortedArraySet with basic types", () => {
  test("infers BasicNumberComparator from first element", () => {
    const set = new LambdaSortedArraySet<number>(undefined, [4, 1, 3, 2]);
  ...
  test("throws ComparatorInferenceError when no comparator and no initial elements", () => {
    expect(() => {
      new LambdaSortedArraySet<number>();
    }).toThrow(ComparatorInferenceError);
  });

  test("ignores duplicate values when adding", () => {
    const set = new LambdaSortedArraySet<number>((left, right) => left - right);
  ...
  test("ignores duplicate values during initialization", () => {
    const set = new LambdaSortedArraySet<number>(undefined, [3, 1, 2, 1]);
  ...
  test("keeps float values sorted with inferred comparator", () => {
    const set = new LambdaSortedArraySet<number>(undefined, [3.14, 2.71, 1.41, 2.5]);

    expect(set.toArray()).toEqual([1.41, 2.5, 2.71, 3.14]);
  });

  test("keeps strings sorted alphabetically with inferred comparator", () => {
    const set = new LambdaSortedArraySet<string>(undefined, ["zebra", "apple", "banana"]);

    expect(set.toArray()).toEqual(["apple", "banana", "zebra"]);
  });

  test("ignores duplicate string when adding", () => {
    const set = new LambdaSortedArraySet<string>(undefined, ["apple", "banana"]);
  ...
  test("contains returns true for existing elements and false for missing", () => {
    const set = new LambdaSortedArraySet<number>((left, right) => left - right, [1, 3, 5]);
  ...
  test("clear removes all elements", () => {
    const set = new LambdaSortedArraySet<number>((left, right) => left - right, [1, 2, 3]);
  ...
  test("uses custom equality comparator when provided", () => {
    const set = new LambdaSortedArraySet<{ id: number; label: string }>(
