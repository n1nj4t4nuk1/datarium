...
describe("LambdaBoundedArrayList with class type User (unsorted)", () => {
  test("keeps insertion order with initial elements", () => {
    ...
    const list = new LambdaBoundedArrayList<User>(3, [user1, user2, user3]);
  ...
  test("add appends users without sorting", () => {
    ...
    const list = new LambdaBoundedArrayList<User>(3, [user1]);
    list.add(user3);
    list.add(user2);
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    ...
    const list = new LambdaBoundedArrayList<User>(2, [user1, user2]);
  ...
  test("addAt inserts user at explicit position", () => {
    ...
    const list = new LambdaBoundedArrayList<User>(3, [user1, user3]);
    list.addAt(1, user2);
