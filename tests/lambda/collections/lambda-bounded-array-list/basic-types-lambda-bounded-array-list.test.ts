...
import { LambdaBoundedArrayList } from "../../../../src/lambda/collections/lambda-bounded-array-list/lambda-bounded-array-list";
...
describe("LambdaBoundedArrayList (unsorted)", () => {
  test("creates an empty list with fixed capacity", () => {
    const list = new LambdaBoundedArrayList<number>(3);
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    const list = new LambdaBoundedArrayList<number>(2, [1, 2]);

    expect(() => list.add(3)).toThrow(ListCapacityExceededError);
  });

  test("addAt inserts at explicit index", () => {
    const list = new LambdaBoundedArrayList<number>(3, [1, 3]);
  ...
  test("set replaces element at index", () => {
    const list = new LambdaBoundedArrayList<number>(3, [1, 2, 3]);
  ...
  test("contains/indexOf/remove use default equality behavior", () => {
    const list = new LambdaBoundedArrayList<string>(3, ["a", "b", "c"]);
  ...
  test("throws IndexOutOfBoundsError for invalid indexes", () => {
    const list = new LambdaBoundedArrayList<number>(1, [1]);
  ...
  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new LambdaBoundedArrayList<number>(-1)).toThrow(InvalidCapacityError);
    expect(() => new LambdaBoundedArrayList<number>(1.5)).toThrow(InvalidCapacityError);
  });
});
