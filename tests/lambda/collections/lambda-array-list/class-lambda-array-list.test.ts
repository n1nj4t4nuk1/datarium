...
describe("LambdaArrayList with class type User (unsorted)", () => {
  test("keeps insertion order with initial elements", () => {
    ...
    const list = new LambdaArrayList<User>([user1, user2, user3]);
  ...
  test("add appends users without sorting", () => {
    ...
    const list = new LambdaArrayList<User>([user1]);
    list.add(user3);
    list.add(user2);
  ...
  test("addAt inserts user at explicit position", () => {
    ...
    const list = new LambdaArrayList<User>([user1, user3]);
    list.addAt(1, user2);
