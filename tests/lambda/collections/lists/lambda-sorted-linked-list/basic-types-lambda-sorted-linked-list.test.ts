...
import { LambdaSortedLinkedList } from "../../../../../src/lambda/collections/lists/lambda-sorted-linked-list/lambda-sorted-linked-list";
import { ComparatorInferenceError } from "../../../../../src/lambda/errors/comparator-inference-error";

describe("LambdaSortedLinkedList with basic types", () => {
  test("infers BasicNumberComparator from first element", () => {
    const list = new LambdaSortedLinkedList<number>(undefined, [4, 1, 3, 2]);
  ...
  test("throws ComparatorInferenceError when no comparator and no initial elements", () => {
    expect(() => {
      new LambdaSortedLinkedList<number>();
    }).toThrow(ComparatorInferenceError);
  });

  test("keeps numbers sorted ascending with explicit comparator", () => {
    const list = new LambdaSortedLinkedList<number>((left, right) => left - right);
  ...
  test("set keeps sorted order after replacing element", () => {
    const list = new LambdaSortedLinkedList<number>((left, right) => left - right, [1, 3, 5]);
  ...
  test("throws IndexOutOfBoundsError when set uses invalid index", () => {
    const list = new LambdaSortedLinkedList<number>((left, right) => left - right, [1]);
  ...
  test("uses inferred equality comparator for number contains/indexOf/remove", () => {
    const list = new LambdaSortedLinkedList<number>(undefined, [3, 1, 2]);
  ...
  test("uses custom equality comparator when provided", () => {
    const list = new LambdaSortedLinkedList<{ id: number; label: string }>(
