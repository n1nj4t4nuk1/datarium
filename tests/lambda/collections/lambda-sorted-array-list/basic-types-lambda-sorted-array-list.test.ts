...
describe("LambdaSortedArrayList with basic types", () => {
  test("infers BasicNumberComparator from first element", () => {
    const list = new LambdaSortedArrayList<number>(undefined, [4, 1, 3, 2]);
  ...
  test("throws ComparatorInferenceError when no comparator and no initial elements", () => {
    expect(() => {
      new LambdaSortedArrayList<number>();
    }).toThrow(ComparatorInferenceError);
  });

  test("keeps numbers sorted ascending with explicit comparator", () => {
    const list = new LambdaSortedArrayList<number>((left, right) => left - right);
  ...
  test("set keeps sorted order after replacing element", () => {
    const list = new LambdaSortedArrayList<number>((left, right) => left - right, [1, 3, 5]);
  ...
  test("throws IndexOutOfBoundsError when set uses invalid index", () => {
    const list = new LambdaSortedArrayList<number>((left, right) => left - right, [1]);
  ...
  test("uses inferred equality comparator for number contains/indexOf/remove", () => {
    const list = new LambdaSortedArrayList<number>(undefined, [3, 1, 2]);
  ...
  test("uses custom equality comparator when provided", () => {
    const list = new LambdaSortedArrayList<{ id: number; label: string }>(
