...
import { ArrayQueue } from "../../../../src/core/collections/array-queue/array-queue";
...
describe("ArrayQueue", () => {
  ...
  test("creates a queue from initial elements", () => {
    const queue = new ArrayQueue<number>([1, 2, 3]);
  ...
  test("peek returns front element without removing it", () => {
    const queue = new ArrayQueue<number>([5, 10]);
  ...
  test("toArray returns a shallow copy", () => {
    const queue = new ArrayQueue<number>([1, 2]);
