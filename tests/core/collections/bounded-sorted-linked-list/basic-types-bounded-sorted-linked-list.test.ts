...
import { BoundedSortedLinkedList } from "../../../../src/core/collections/bounded-sorted-linked-list/bounded-sorted-linked-list";
...
describe("BoundedSortedLinkedList", () => {
  test("creates an empty list by default", () => {
    const list = new BoundedSortedLinkedList<number>(3);
  ...
  test("sorts numbers in constructor", () => {
    const list = new BoundedSortedLinkedList<number>(4, [4, 1, 3, 2]);
  ...
  test("keeps strings sorted alphabetically", () => {
    const list = new BoundedSortedLinkedList<string>(3, ["zebra", "apple", "banana"]);

    expect(list.toArray()).toEqual(["apple", "banana", "zebra"]);
  });

  test("addAt ignores provided index and preserves sorting", () => {
    const list = new BoundedSortedLinkedList<number>(3, [10, 30]);
  ...
  test("set preserves sorting and returns previous value", () => {
    const list = new BoundedSortedLinkedList<number>(3, [1, 3, 5]);
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    const list = new BoundedSortedLinkedList<number>(2, [2, 1]);
  ...
  test("throws ListCapacityExceededError when initial elements exceed capacity", () => {
    expect(() => new BoundedSortedLinkedList<number>(2, [3, 2, 1])).toThrow(
      ListCapacityExceededError,
    );
  });

  test("supports contains and indexOf in sorted list", () => {
    const list = new BoundedSortedLinkedList<number>(5, [5, 1, 3]);
  ...
  test("throws IndexOutOfBoundsError for invalid indexes", () => {
    const list = new BoundedSortedLinkedList<number>(1, [1]);
  ...
  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new BoundedSortedLinkedList<number>(-1)).toThrow(InvalidCapacityError);
    expect(() => new BoundedSortedLinkedList<number>(1.5)).toThrow(InvalidCapacityError);
  });
});
