...
import { BoundedArrayQueue } from "../../../../src/core/collections/bounded-array-queue/bounded-array-queue";
...
describe("BoundedArrayQueue", () => {
  test("creates an empty bounded queue by default", () => {
    const queue = new BoundedArrayQueue<number>(3);
  ...
  test("creates a bounded queue from initial elements", () => {
    const queue = new BoundedArrayQueue<number>(3, [1, 2, 3]);
  ...
  test("enqueue adds elements while capacity allows", () => {
    const queue = new BoundedArrayQueue<number>(2);
  ...
  test("throws ListCapacityExceededError when enqueueing beyond capacity", () => {
    const queue = new BoundedArrayQueue<number>(2, [1, 2]);
  ...
  test("throws ListCapacityExceededError when initial elements exceed capacity", () => {
    expect(() => new BoundedArrayQueue<number>(2, [1, 2, 3])).toThrow(ListCapacityExceededError);
  });
  ...
  test("peek returns front element without removing it", () => {
    const queue = new BoundedArrayQueue<number>(2, [5, 10]);
  ...
  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new BoundedArrayQueue<number>(-1)).toThrow(InvalidCapacityError);
    expect(() => new BoundedArrayQueue<number>(1.5)).toThrow(InvalidCapacityError);
  });
});
