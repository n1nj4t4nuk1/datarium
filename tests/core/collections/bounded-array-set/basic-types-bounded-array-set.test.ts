...
import { BoundedArraySet } from "../../../../src/core/collections/bounded-array-set/bounded-array-set";
...
describe("BoundedArraySet", () => {
  test("creates an empty set by default", () => {
    const set = new BoundedArraySet<number>(3);
  ...
  test("throws DuplicateElementError when initial values contain duplicates", () => {
    expect(() => new BoundedArraySet<number>(3, [3, 1, 2, 2, 1])).toThrow(DuplicateElementError);
  });

  test("add inserts new values and throws for duplicates", () => {
    const set = new BoundedArraySet<number>(2);
  ...
  test("contains works correctly", () => {
    const set = new BoundedArraySet<string>(2, ["b", "a"]);
  ...
  test("remove deletes existing value and throws when absent", () => {
    const set = new BoundedArraySet<number>(3, [1, 2, 3]);
  ...
  test("throws ListCapacityExceededError when adding beyond capacity", () => {
    const set = new BoundedArraySet<number>(2, [1, 2]);

    expect(() => set.add(3)).toThrow(ListCapacityExceededError);
  });

  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new BoundedArraySet<number>(-1)).toThrow(InvalidCapacityError);
    expect(() => new BoundedArraySet<number>(1.5)).toThrow(InvalidCapacityError);
  });
