...
import { BoundedLinkedStack } from "../../../../src/core/collections/bounded-linked-stack/bounded-linked-stack";
...
describe("BoundedLinkedStack", () => {
  test("creates an empty bounded stack by default", () => {
    const stack = new BoundedLinkedStack<number>(3);
  ...
  test("creates a bounded stack from initial elements", () => {
    const stack = new BoundedLinkedStack<number>(3, [1, 2, 3]);
  ...
  test("throws ListCapacityExceededError when pushing beyond capacity", () => {
    const stack = new BoundedLinkedStack<number>(2, [1, 2]);
  ...
  test("throws ListCapacityExceededError when initial elements exceed capacity", () => {
    expect(() => new BoundedLinkedStack<number>(2, [1, 2, 3])).toThrow(ListCapacityExceededError);
  });
  ...
  test("peek returns top element without removing it", () => {
    const stack = new BoundedLinkedStack<number>(2, [5, 10]);
  ...
  test("throws InvalidCapacityError for invalid capacity", () => {
    expect(() => new BoundedLinkedStack<number>(-1)).toThrow(InvalidCapacityError);
    expect(() => new BoundedLinkedStack<number>(1.5)).toThrow(InvalidCapacityError);
  });
});
